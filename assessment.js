...
var thirdHero = heroes.slice(1,-1).toString();
...
//It looks to see if the person’s name is Jeremy and then changes the person object to have a property
//called lovesCode with a value of 10.  If their name is Brack set lovesCode to 0.  otherwise set lovesCode to 5.

function coderTest(object){
	object.lovesCode = 5;
	if(object.name === 'Jeremy'){
		object.lovesCode += 5;
		//console.log(object);
	}
	if(object.name === 'Brack'){
		object.lovesCode = 0;
	}
...
function outside(temp,humid,cloud){
	...
	if(temp > 80 && humid < 40 && cloud < 20){
		return "I'm literally in the desert";
	}
	if(temp > 80 || humid > 50 || cloud > 50){
